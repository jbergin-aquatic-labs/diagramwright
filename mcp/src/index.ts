...
async function main(): Promise<void> {
  ...
  const mode = (process.env.MCP_TRANSPORT ?? "stdio").toLowerCase() as Transport;
  ...
  try {
    ...
    log(
      `Failed to start sync bridge on port ${bridgePort}:`,
  ...
  if (mode === "http" || mode === "both") {
    try {
      ...
      log(
        `Failed to start MCP HTTP endpoint on port ${httpPort}:`,
        err instanceof Error ? err.message : err,
      );
    }
  }
