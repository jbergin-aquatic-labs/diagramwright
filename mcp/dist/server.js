...
export function createMcpServer(hub) {
    ...
    server.registerTool("get_diagram", {
        title: "Get diagram",
        description: "Return the current diagram as JSON (nodes, edges, direction, title) plus the live revision number.",
        inputSchema: {},
    }, async () => {
        const snap = hub.snapshot();
        return text(JSON.stringify(snap, null, 2));
    });
    server.registerTool("get_mermaid", {
        title: "Get Mermaid",
    ...
    server.registerTool("add_node", {
        title: "Add node",
        description: "Add a flowchart node. kind is one of process, decision, terminal, database, subroutine. Returns the new node id.",
        inputSchema: {
            kind: kindEnum,
    ...
    server.registerTool("add_edge", {
        title: "Add edge",
        description: "Connect two existing nodes by their ids, with an optional edge label.",
        inputSchema: {
            source: z.string(),
            target: z.string(),
            label: z.string().optional(),
    ...
    server.registerTool("remove_edge", {
        title: "Remove edge",
    ...
    server.registerTool("set_diagram", {
        title: "Set diagram",
        description: "Replace the entire diagram with a full JSON document. Use for bulk edits or generating a diagram from scratch.",
        inputSchema: { diagram: diagramSchema },
    }, async ({ diagram }) => {
        hub.setDiagram(diagram, "agent");
        return ok("Replaced the diagram.");
    });
    server.registerTool("clear_diagram", {
        title: "Clear diagram",
    ...
    server.registerResource("current-diagram", "diagram://current", {
    ...
    server.registerResource("current-mermaid", "diagram://mermaid", {
